...
struct formatter<elink::iec60870::details::InformationObjectSerializable<inherit, typeID>> : formatter<std::string_view>
{
    ...
    auto format(const IO& io, FormatContext& ctx) const
    {
        ...
        *out++ = '{';
        ...
        if (intuitive)
        {
            std::format_to(out, R"(: ioa={} data=[)", io.getAddress());
        }
        else
        {
        ...
        for (std::size_t i = intuitive ? io.getAddress().length() : 0; i < io.size(); ++i)
        {
...
#include "elink/iec60870/formatter/IOList.hpp"
...
ELINK_IO_FORMATTER(SetpointCommandShortWithCP56Time2a);
ELINK_IO_FORMATTER(BitString32CommandWithCP56Time2a);
...
ELINK_IO_FORMATTER(DelayAcquisitionCommand);
ELINK_IO_FORMATTER(TestCommandWithCP56Time2a);
